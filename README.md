...
- **✉️ Oyuncuya DM (/dm-player)** - bir oyuncuya **AC'nin kendi hesabından** bot
  ...
  gönderilir. **Hız:** DM komutu ve kanalı, panel bağlıyken `/api/ac/hazirla`
  ısıtmasında önceden çözülüp önbelleğe alınır, böylece ilk DM de hızlı gider.
- **🎫 AC Ticket Aç** - bir kişinin **Discord ID**'sini gir; **AC'nin kendi
  hesabından** sabit kanala (`1470230489456578759`)
  `<@girilen> Ac Ticket Aç <@AC>` mesajı atar (önce girilen kişi, sonra gönderen
  AC etiketli). Ticket seçmene gerek yok.
...
| **Gateway YALNIZCA slash için (sıcak tutulur)** | **Hız:** Düz mesaj gönderen her şey (karşılama, **Kirli** sonucu, **AC Ticket Aç**, **GIF**) tek bir **REST** isteğiyle gider - gateway login beklemesi yok, anında. Yalnızca gerçek slash komutlar (`/nexorapin`, `/dm-player`) **canlı bir gateway oturumu** gerektirir; bu yüzden **Nexora At**/DM'de (ya da ticket seçilince) o AC için geçici bir selfbot bağlantısı açılır. **Hız için:** Nexora Panel açıkken bağlantı ~8 dk'da bir ısıtılıp **sıcak tutulur**, böylece AC ticket'a anahtar kelimesini yazınca `/nexorapin` **anında** gider (soğuk bağlantı beklemesi olmaz). Panel kapanınca bağlantı **12 dk boşta kalınca kendiliğinden kapanır**. `/nexorapin` komutunun kimliği (application_id + ad) 30 dk **önbelleğe** alınır - her tetikte yeniden çözülmez. Aynı anda en çok 15 AC bağlantısı tutulur; kalıcı 7/24 selfbot yok. |
...
| `POST /api/ac/ticket-ac` | AC'nin kendi hesabından sabit kanala `<@id> Ac Ticket Aç <@AC>` (kişi + AC etiketli) |
